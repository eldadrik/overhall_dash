...
function emptyStockTrendBucket(error) {
  return {
    status: "error",
    caption: `לא ניתן לקרוא את Google Trends Explore כרגע: ${error.message}`,
    items: []
  };
}

async function loadGoogleTrendsData({ mode = "regular" } = {}) {
  const warnings = [];
  const isStockMode = mode === "stocks";

  const [searchHe, searchEn] = await Promise.all([
    settleBucket(
      () => getGoogleTrendsBucket("he", { mode }),
      (error) => (isStockMode ? emptyStockTrendBucket(error) : emptyTrendBucket("מגמות החיפוש בעברית")),
      warnings,
      isStockMode ? "Google Trends מניות / ישראל" : "Google Trends / ישראל"
    ),
    settleBucket(
      () => getGoogleTrendsBucket("en", { mode }),
      (error) => (isStockMode ? emptyStockTrendBucket(error) : emptyTrendBucket("מגמות החיפוש באנגלית")),
      warnings,
      isStockMode ? "Google Trends Stocks / ארה״ב" : "Google Trends / ארה״ב"
    )
  ]);
  ...
  return {
    ...
    warnings,
    summary: {
      activePanels: [searchHe, searchEn].filter(isActiveBucket).length
    },
    searches: {
      he: mapTrendBucket(searchHe),
      en: mapTrendBucket(searchEn)
    },
    sources: {
      searches: {
...
async function loadDashboardData() {
  ...
  const [searchHe, searchEn, hotBet] = await Promise.all([
    settleBucket(() => getGoogleTrendsBucket("he", { mode: "regular" }), () => emptyTrendBucket("מגמות החיפוש בעברית"), warnings, "Google Trends / ישראל"),
    settleBucket(() => getGoogleTrendsBucket("en", { mode: "regular" }), () => emptyTrendBucket("מגמות החיפוש באנגלית"), warnings, "Google Trends / ארה״ב"),
...
export async function getGoogleTrendsData({ mode = "regular", forceRefresh = false } = {}) {
  const normalizedMode = mode === "stocks" ? "stocks" : "regular";

  if (normalizedMode === "stocks") {
    return loadGoogleTrendsData({ mode: normalizedMode });
  }

  return withTtlCache(`google-trends-${normalizedMode}-v1`, CACHE_TTL_MS, () => loadGoogleTrendsData({ mode: normalizedMode }), {
    forceRefresh
  });
}

export async function getDashboardData({ forceRefresh = false } = {}) {
  return withTtlCache("dashboard-v6", CACHE_TTL_MS, () => loadDashboardData(), { forceRefresh });
}
