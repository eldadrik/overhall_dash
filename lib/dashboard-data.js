...
async function loadDashboardData() {
  ...
  const [searchHe, searchEn, xHe, xEn, hotBet] = await Promise.all([
    ...
    settleBucket(() => getXTrendsBucket("he"), () => emptyTrendBucket("מגמות X בעברית"), warnings, "X / ישראל"),
    settleBucket(() => getXTrendsBucket("en"), () => emptyTrendBucket("מגמות X באנגלית"), warnings, "X / ארה״ב"),
...
export async function getDashboardData({ forceRefresh = false } = {}) {
  return withTtlCache("dashboard-v1", CACHE_TTL_MS, loadDashboardData, { forceRefresh });
}
