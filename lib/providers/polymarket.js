...
function isCryptoMarket(market) {
  ...
  return CRYPTO_KEYWORDS.some((keyword) => hasKeyword(text, keyword));
}
...
function scoreMarket(market, historyByTokenId) {
  ...
  const score = clamp01(
    SCORE_WEIGHTS.uncertainty * uncertaintyScore +
      SCORE_WEIGHTS.priceMove * priceMove.score +
      SCORE_WEIGHTS.attention * attentionShockScore +
      SCORE_WEIGHTS.urgency * urgencyScore +
      SCORE_WEIGHTS.topic * topicScore -
      SCORE_WEIGHTS.badLiquidityPenalty * badLiquidityPenalty -
      SCORE_WEIGHTS.sportsPenalty * sportsPenalty
  );
...
function normalizePriceHistory(history, pair) {
  const nowSeconds = Math.floor(Date.now() / 1000);
  const earliestSeconds = nowSeconds - ONE_WEEK_SECONDS;
  const points = (Array.isArray(history) ? history : [])
    .map((point) => ({
      t: Number(point.t),
      p: Number(point.p)
    ...
    .filter((point) => point.t >= earliestSeconds && point.t <= nowSeconds + 60)
    .sort((left, right) => left.t - right.t)
    .map((point) => ({
      time: new Date(point.t * 1000).toISOString(),
      price: clamp01(point.p)
    }));

  if (!points.length && Number.isFinite(pair.price)) {
    points.push({
      time: new Date().toISOString(),
      price: clamp01(pair.price)
    });
  }
...
function normalizeMarket(scoredMarket, history) {
  const { market, pair, components, score } = scoredMarket;
  ...
  return {
    ...
    priceHistory: normalizePriceHistory(history, pair)
  };
}
...
export async function getHotBetBucket() {
  const markets = (await fetchCandidateMarkets())
    ...
    .filter((market) => !isCryptoMarket(market))
  ...
  return {
    ...
    item: normalizeMarket(scoredMarket, historyByTokenId[scoredMarket.pair.tokenId]),
    caption: "Selected by Interesting Score: uncertainty, price move, attention shock, topic, reduced urgency weight, liquidity, and sports/spam penalty. Crypto markets are excluded."
  };
}
